...
int main() {
    ...
    int pipe_awk_cat[2];
    int pipe_cat_cat[2];
    pid_t child_ls, child_awk, child_cat0, child_cat;

    // Create pipes
    if (pipe(pipe_ls_awk) == -1 ||pipe(pipe_awk_cat) == -1 || pipe(pipe_awk_cat) == -1) {
        perror("pipe");
        exit(EXIT_FAILURE);
    }
    ...
    if ((child_cat0 = fork()) == -1) {
        perror("fork");
        exit(EXIT_FAILURE);
    }

    if (child_cat0 == 0) { // Child process (cat)
        // Close unnecessary pipe ends
        close(pipe_ls_awk[0]);
        close(pipe_ls_awk[1]);
        close(pipe_awk_cat[1]);
